...
var (
	// The directories and files where "xcweaver ssh" stores data.
	//
...
type manager struct {
	...
	// to store the xcweaver binaries at each location
...
func (m *manager) startBabysitter(loc string, info *BabysitterInfo) error {
	...
	binaryPath := filepath.Join(m.locations[loc], "xcweaver")
